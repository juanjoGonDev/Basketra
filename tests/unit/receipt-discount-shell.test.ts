...
test('service worker versions the shell and receipt review owns typed discount styles', () => {
  ...
  assert.match(serviceWorker, /const CACHE_PREFIX = 'basketra-shell-'/u);
  assert.match(serviceWorker, /searchParams\.get\('version'\)/u);
  assert.match(serviceWorker, /'\/receipt-review\.css'/u);
  assert.match(serviceWorker, /'\/receipt-editor-invoice\.js'/u);
  assert.match(serviceWorker, /'\/receipt-editor-invoice\.css'/u);

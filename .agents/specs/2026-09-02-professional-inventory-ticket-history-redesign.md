...
Implemented regression coverage now includes navigation/Plans removal, static-asset serving, v10 upgrade, Store management, Statistics aggregation, ticket evidence preservation, exact historical value parsing, API-owner uniqueness, composite route resolution, Ticket History mobile/desktop flows and deep links, Store mobile detail/edit/delete impact, Statistics accessible table equivalents, stale-period response protection and no-horizontal-overflow assertions/screenshots in the new Playwright scenarios.

These new tests are committed but have not been executed on the PR #51 head. The canonical `Pull Request Quality` and CodeQL workflows are configured only for pull requests whose base is `main`; PR #51 is intentionally stacked on `agent/feat-ticket-category-classification` / PR #49, which remains open. A local reconstruction was attempted from the exact GitHub head, but this execution environment cannot resolve `github.com`/`codeload.github.com`, so `pnpm quality` and Playwright cannot be truthfully reported as run here. Final delivery remains blocked on authoritative Quality, Security, Browser E2E, container/CodeQL and screenshot inspection once the stacked-base dependency permits those workflows.

## Status

Runtime implementation is in progress on `agent/feat-professional-inventory-redesign`. Plans runtime/sentinel code has been removed; Inventory Products/Categories/Stores/Statistics and Ticket History have dedicated runtime surfaces and server boundaries; Stores/Statistics and historical-ticket workflows have additional regression coverage; and historical evidence safety has a single canonical API owner. The six approved PNG files remain unchanged and are still the normative visual authority. Final pixel-parity review, full regression execution, CI evidence and any resulting fixes remain open. No merge, release or deploy has been performed.

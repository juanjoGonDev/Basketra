...
Runtime implementation and affected-screen visual review are complete on `agent/feat-professional-inventory-redesign`; PR #51 remains open, non-draft and mergeable. Plans runtime/sentinel code is removed; Inventory Products/Categories/Stores/Statistics and Ticket History have dedicated runtime surfaces and server boundaries; historical evidence safety has a single canonical API owner; the final Browser suite is 106/106 on the last runtime head; the Ticket History desktop header regression is fixed and visually confirmed; and the visual-evidence workflow selector has been repaired. The remaining gate is exact-head CI plus successful direct visual-evidence publication after the final workflow/documentation commits, followed by PR-body synchronization and one last PR/CI/diff review. The six approved PNG files remain unchanged and authoritative. No merge, release or deploy has been performed.

...
Regression coverage includes navigation/Plans removal, static-asset serving, schema upgrade through v11, Store management, Statistics aggregation, ticket evidence preservation, exact historical value parsing, API-owner uniqueness, composite route resolution, Ticket History mobile/desktop flows and deep links, Store mobile detail/edit/delete impact, Statistics accessible table equivalents, stale-period response protection and no-horizontal-overflow assertions/screenshots in the new Playwright scenarios.

After rebasing the dependency relationship onto the final PR #49 head, exact head `b7ecab0ee8e2d65a102a892d851f6ca8b8c081a8` passed Quality, Security, resource/growth budgets, container smoke, linux/amd64, linux/arm64 and CodeQL. Browser E2E run `33763403348` executed 101 scenarios: 61 passed and 40 failed. Inspection of the exact screenshots and logs reduced those failures to two root causes rather than 40 independent defects: the new application-readiness gate allowed primary-navigation clicks to return before the requested route was actually activated/receipt handlers were ready, and category save completion could reopen a detail after the user had already returned to the list.

The receipt failure screenshots showed the Tickets shell eventually active but no capture card/review transition after the earlier interaction had already been lost. Comparison with the final PR #49 `app.js` confirmed that the readiness gate was introduced by this redesign. Existing successful receipt scenarios that waited for a post-navigation state also confirmed that OCR/AI/file persistence itself was not universally broken. The category failure similarly matched the asynchronous `saveCategory()` sequence: PATCH completion refreshed metadata/inventory and then unconditionally called `openCategoryDetail(savedId)`.

Regression commits now cover both races: `79175bb0ffd80262ddbde5503248a612d961e75b` adds the delayed category-save navigation test, `968c7217a4a560850c58f032e3d33f48ed758784` adds the application-readiness navigation contract, `5f0395bdd9b737f13f61d299d8f317779d69569f` gates primary navigation while bootstrap is busy, and `3139783b548870475ace9da07973a1ef8c14b438` prevents stale category-save completion from taking over a newer route. The runtime diff from `b7ecab0e` to `3139783b` is limited to three lines in `src/web/app.js` and two lines in `src/web/catalog.js`, plus the two focused regression tests. Exact-head CI for the post-fix/documentation head remains required.

## Status

Runtime implementation remains in progress on `agent/feat-professional-inventory-redesign`. PR #51 is stacked directly on final PR #49 and is mergeable; Plans runtime/sentinel code has been removed; Inventory Products/Categories/Stores/Statistics and Ticket History have dedicated runtime surfaces and server boundaries; Stores/Statistics and historical-ticket workflows have regression coverage; historical evidence safety has a single canonical API owner; and the two Browser regressions found after the dependency merge have focused tests and fixes. The six approved PNG files remain unchanged and are still the normative visual authority. Exact-head CI, compact visual-evidence publication and final pixel-parity review of every approved reference remain open. No merge, release or deploy has been performed.

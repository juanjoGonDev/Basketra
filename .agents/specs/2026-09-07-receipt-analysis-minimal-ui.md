...
- Touch targets remain at least the existing `--touch` target; focus remains visible; `prefers-reduced-motion` is respected by slowing the functional processing spinner while disabling the non-essential error pulse.
...
- A continuously rotating progress affordance can distract or affect motion-sensitive users. Mitigation: animate only during real work, use a short gradient arc rather than a full ring, stop immediately on complete/error/idle, keep the inner icon stable, and slow the same functional spinner from 0.8 s to 1.6 s per turn under `prefers-reduced-motion` instead of freezing it; the non-essential red error pulse remains disabled under reduced motion.
...
- The rotating arc is rendered as a dedicated spinner layer inside the floating control. Its CSS transform rotation must visibly change between successive animation frames; the receipt icon/count must not rotate with it. Under reduced motion, the same partial arc remains visible and rotates at half speed so the processing state remains explicit without relying on the normal-speed motion.
...
30. The collapsed floating source/progress control shows a non-blocking rotating partial gradient arc (roughly one third of the circumference, never a full ring) only while aggregate state is `working`; completed/idle/error states do not use that spinner. The inner receipt icon and count remain stationary. Rotation is defined explicitly with `@keyframes ... { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`. With `prefers-reduced-motion: reduce`, the same arc still rotates at half speed so processing remains unmistakable.
...
- Spinner delivery follow-up head `8ed3a159678bd2713ba46e6983505d94da9dd4f3` passed Pull Request Quality run `34280280832` and CodeQL Advanced run `34280280801` with no failed jobs; Browser 49/56, Browser coverage and `CI complete` all passed.
- Exact-head Browser 49/56 evidence was re-reviewed after the delivery fix: `receipt-progressive-collapsed-390.png` shows the partial green processing arc with stationary receipt/count, `receipt-progressive-queue-390.png` keeps file/global progress bounded in the disclosure, and `receipt-progress-error-pulse-390.png` preserves the error-state layout without horizontal overflow or clipping.
- The only production change after the approved UI head is the Service Worker freshness rule for `/receipt-review.css`; no receipt layout, spinner geometry, API, persistence or dependency code changed.
...
- Exact-head Browser 49/56 passed the processing-state regression: the working partial gradient arc is present, its computed transform changes between animation frames, the inner receipt/count remain stationary, reduced-motion rotates the same arc at half speed, and the error state retains the receipt glyph with the slow red perimeter pulse contract.
...
- Browser spinner regression on `d9d4f4087a69fadcf762766e98975c19506cc854` exposed that the repository-wide reduced-motion rule could clamp the animation to effectively zero. The follow-up makes the working selector own the explicit `from`/`to` transform animation directly and applies a higher-specificity 1.6 s rotation under reduced motion; Browser coverage verifies frame-to-frame transform progression in both modes.
...
- [x] PR created.
- [x] Production/UI head `b22ffa18fc845d32013fb45bb3f624f1493ebc07` and spinner-delivery implementation head `8ed3a159678bd2713ba46e6983505d94da9dd4f3` are green; final documentation-only closure CI is recorded in the PR body.
- [x] Final request/spec/diff/visual review complete for the approved responsive redesign and spinner delivery follow-up.

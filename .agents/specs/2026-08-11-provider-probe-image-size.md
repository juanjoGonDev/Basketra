...
- PR #26 now stores the OCR challenge as `src/ai/fixtures/provider-probe.jpg`, but `src/ai/provider.ts` still sends those JPEG bytes as `test.png`, declares `format: png`, uses `data:image/png`, and retains PNG-specific symbol names.
- `tests/unit/provider-probe-contract.test.ts` points at the JPEG file but still parses/asserts PNG bytes, so its contract no longer represents the real fixture.
- The latest Pull Request Quality run fails because `@types/node@26` was added to a Node 22 project while `src/types/*.d.ts` already owns intentionally minimal Node ambient declarations. The branch also expanded the root `tsconfig.json` to type-check every test, exposing unrelated legacy test typings that the canonical source typecheck did not previously own.
...
- No current runtime/test/spec identifier falsely describes the JPEG probe as PNG; historical completed PNG task specifications remain intact.
...
Root causes confirmed from the failing exact-head CI. Implementation and final CI validation pending.

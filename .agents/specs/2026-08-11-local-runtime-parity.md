...
- Native `pnpm dev` fails before application startup with a clear remediation when Node is below 22.16 or FTS5 is unavailable.
...
- unit contract for package scripts and Docker host-gateway mapping
...
The native runtime probe deliberately rejects unsupported local environments instead of allowing a later SQLite migration failure. The Docker host-gateway alias exposes only the host address from inside the container; it does not publish additional host ports or change Basketra's loopback bind.

## Rollback

Revert the development-tooling commit. Persistent data and schema require no rollback.
